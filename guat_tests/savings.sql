...
SELECT * FROM `mifos-guat`.m_savings_account_transaction; #NULL.

SELECT * FROM guat.savingsaccount; #NULL
SELECT * FROM guat.savingstransaction; #NULL

SHOW COLUMNS FROM `mifos-guat`.m_savings_account;
SHOW COLUMNS FROM guat.savingsaccount;

SHOW CREATE TABLE `mifos-guat`.m_savings_account;
...
SELECT
	...
	,COUNT(INTERESTCHARGEFREQUENCY)				AS freq
	,COUNT(DISTINCT INTERESTCHARGEFREQUENCY)		AS dist_freq
	,COUNT(*) - COUNT(INTERESTCHARGEFREQUENCY)		AS null_freq
...
FROM
	guat.savingsaccount									AS sa
	,`mifos-guat`.m_savings_account			AS m_sav
WHERE
